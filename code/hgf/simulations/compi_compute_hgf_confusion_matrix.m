...
%--------------------------------------------------------------------------
% Function that computes model confusion matrix using the VBA toolbox.
%--------------------------------------------------------------------------
...
for n = noise
    ...
    for idx_seed = 1:n_seeds
        ...
        plot_C(C(:,:,idx_seed), {options.hgf.model_names{selected_models}}, plot_title, 'off');
    ...
    if options.hgf.model_space==1
         saveas(gcf,fullfile(options.roots.figures, [save_name '_av.png']));
    end
